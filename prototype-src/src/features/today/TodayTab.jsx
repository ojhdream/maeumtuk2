...
export default function TodayTab({ stage, count, entries, onOpenEntry, onCycle, onGoTab }) {
  ...
  return (
    <section className="view active today-screen today-alive today-refined">
      <Brand action="↻" onAction={onCycle} />

      <header className="today-hero alive-hero refined-hero">
        <div className="date">5월 20일 화요일</div>
        <h1 className="hero today-hero-title alive-title refined-title">
          지금,
          <br />
          무엇이 <span className="accent">툭</span> 떠올랐나요?
        </h1>
        <p className="guide-copy today-guide alive-guide refined-guide">
          문득 든 생각,
          <br />
          한 단어여도 괜찮아요.
        </p>

        <div className="today-maeumi-stage refined-maeumi-stage" aria-hidden="true">
          <img className="maeumi-base-img" src="./maeumi-base.svg" alt="" />
        </div>
      </header>

      <button type="button" className="write-hero panel today-write-card alive-write-card refined-write-card" onClick={onOpenEntry}>
        <span className="alive-write-mark refined-write-mark" aria-hidden="true">
          <i></i>
          <b></b>
        </span>
        <span className="today-write-copy">
          <strong>{hasEntries ? '툭 남기기' : '첫 툭 남기기'}</strong>
          <span>지금 떠오른 마음을 한 조각 남겨보세요.</span>
        </span>
        <span className="refined-card-arrow" aria-hidden="true">›</span>
      </button>
...
function ZeroState({ count, onGoTab }) {
  return (
    <>
      <section className="today-empty-note panel refined-empty-note">
...
function MiniProgress({ count, onGoTab }) {
  ...
  return (
    <section className="today-mini-progress alive-discovery-card refined-discovery-card" onClick={() => onGoTab('now')}>
      <h2>
        기록은 내가 하고,
        <br />
        발견은 마음툭이 해요.
      ...
      <div className="refined-discovery-body">
        <p>
          {isReady
...
function TodayList({ entries, onGoTab }) {
  ...
  return (
    <>
      <div className="section-head today-section-head alive-section-head refined-section-head">
        <div>
          <h2>오늘 남긴 툭</h2>
          <p>오늘의 조각들</p>
        </div>
      ...
      <div className="today-list compact alive-today-list refined-today-list">
        {entries.map((entry) => (
